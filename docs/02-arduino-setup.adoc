...
On Linux you can do this with the following command:

   $ ln -sf libraries/* ~/Arduino/libraries/


=== External libraries

All external libraries we depend upon for the firmware are included in the repository.
This to ensure that we have reproducible builds. Hence there is no need to download them separately.
