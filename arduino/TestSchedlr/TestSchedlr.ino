...
void flip3(void *)
{
  ...
  __LOG(" :flip3: ");
  __LOGLN(state);
  __LOG("last "); __LOG(T3->last_time); __LOG(" next ");    __LOG(T3->next_time); __LOG("  period "); __LOGLN(T3->next_time - T3->last_time);
  ++count3;
  if (count3 > 10)
  {
    __LOG(" period for task3  is ");
    __LOGLN(T3->period);
    T3->replaceDelayed(&flip3, T3->period * 20);
    __LOG("last "); __LOG(T3->last_time); __LOG(" next ");    __LOG(T3->next_time); __LOG("  period "); __LOGLN(T3->next_time - T3->last_time);
    count3 = 0;
  }
}
...
void setup() {
  ...
  __LOG("Task 0 next time is "); __LOGLN(T1->next_time);
  __LOG("Task 0 period is "); __LOGLN(T1->period);
  __LOG("Task 0 type is "); __LOGLN(T1->tasktype);
  __LOG("Schedule time "); __LOGLN(schedule->getTickTime());
  __LOG("Task 1 next time is "); __LOGLN(T2->next_time);
  __LOG("Task 1 period is "); __LOGLN(T2->period);
  __LOG("Task 1 type is "); __LOGLN(T2->tasktype);
  //__LOG("Schedule time "); __LOGLN(schedule->getTickTime());
  __LOG("Task 2 next time is "); __LOGLN(T3->next_time);
  __LOG("Task 2 period is "); __LOGLN(T3->period);
  __LOG("Schedule time "); __LOGLN(schedule->getTickTime());
